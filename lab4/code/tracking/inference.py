...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(len(L)):
        for j in range(len(L)):
            if  (abs(manhattanDistance(L[i],L[j])+MAX_NOISE)) not in MAN:
                MAN.append(abs(manhattanDistance(L[i],L[j])+MAX_NOISE))
            if  (abs(manhattanDistance(L[i],L[j])-MAX_NOISE)) not in MAN:
                MAN.append(abs(manhattanDistance(L[i],L[j])-MAX_NOISE))
    variableDomainsDict[OBS0]=MAN
    variableDomainsDict[OBS1]=MAN
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        f_list = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for join_v in eliminationOrder:
            f_list, join_f = joinFactorsByVariable(f_list, join_v)
            if (len(join_f.unconditionedVariables())> 1):
                eliminated_f = eliminate(join_f, join_v)
                f_list.append(eliminated_f)
        return normalize(joinFactors(f_list))
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
